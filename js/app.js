...
function connectSocket(andMatch) {
  ...
  // Private room events
  socket.on('room_created', async ({ code }) => {
    // Show the code on the private page — DON'T switch away yet
    document.getElementById('createdCode').textContent = code;
    document.getElementById('privateStatus').textContent = '⏳ Waiting for your friend to join with this code...';

    // Pre-warm: get camera + gayometer ready in background, but stay on private page
    try {
      if (!localStream) {
        localStream = await navigator.mediaDevices.getUserMedia({
          video:{ width:{ideal:1280}, height:{ideal:720}, facingMode:'user' },
    ...
    } catch(e) { /* camera will be requested again when match starts */ }
    setStatus('searching', `🔐 Room ${code} ready — waiting for friend...`);
  });
...
function endAndRate() {
  ...
  cleanup(false);  // keep camera alive for possible rematch
  showResults(my, str);
}
...
function startRematch() {
  ...
  const startCamera = async () => {
    try {
      ...
      addSys('⚠ Camera error on rematch.');
    }
  };
  startCamera();
}
